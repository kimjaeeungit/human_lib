	            ...
	            <div class="managerBoardArea mx-auto">
	                <div class="managerBoardBox px-2 py-5">
	                    <div class="bookListTitle clearfix pb-2 border-bottom border-2 border-info">
	                        <h5 class="float-left text-info">대출 예약 신청 현황</h5>
	                    </div>
							...
							<table class="table table-hover">
								<thead class="bg-dark text-light">
									...
									<tr>
										...
										<th class="text-center small border-right-0 bg-primary align-middle">픽업 처리</th> 
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${list}" var="loans">
										<tr>
											...
											<td class="text-center small align-middle">예약신청</td>
											<td class="text-center align-middle">
												<button type="button" class="btn btn-outline-success btn-sm small d-inline" onclick="acceptLoans(${loans.lno})">확정</button>
												<button type="button" class="btn btn-outline-warning btn-sm small d-inline" onclick="cancelLoansReservationByManager(${loans.lno}, ${loans.bkno},'${loans.id}')">불가</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
						...
						<div class="col text-center">
							<div class="managerListPagination btn-group">
								<c:if test="${page.prev}">
									<button type="button" class="btn btn-outline-primary" onclick="location.href='${pageContext.request.contextPath}/loans/listLoansReservationRequest?pageNum=${page.startPage-1}&amount=${page.cri.amount}'">
										<i class="fas fa-angle-left"></i> 이전
									</button>
								</c:if>
								<c:forEach begin="${page.startPage}" end="${page.endPage}" var="p">
									<button type="button" class="btn btn-outline-primary ${p == page.cri.pageNum ? 'active' : ''}" onclick="location.href='${pageContext.request.contextPath}/loans/listLoansReservationRequest?pageNum=${p}&amount=${page.cri.amount}'">
										${p}
									</button>
								</c:forEach>
								<c:if test="${page.next}">
									<button type="button" class="btn btn-outline-primary" onclick="location.href='${pageContext.request.contextPath}/loans/listLoansReservationRequest?pageNum=${page.endPage+1}&amount=${page.cri.amount}'">
...
<script>
// 대출 예약 확정 by Manager
function acceptLoans(lno) {
	<sec:authorize access="hasAnyRole('ROLE_ANONYMOUS', 'ROLE_MEMBER')">
		if(confirm("대출예약 확정은 관리자만 가능합니다."))
			location.href="${pageContext.request.contextPath}/";
		else return;
	</sec:authorize>
	if (confirm("해당 "+ lno +"번 대출예약 신청 건을 픽업완료하여 예약을 확정하시겠습니까?")) {
		$("#LoadingLayerModal").modal({keyboard: false, backdrop: 'static'});
		var loans = {lno: lno};
		loansService.acceptLoans(loans, function(result) {
...
// 대출 예약 취소 by Manager
function cancelLoansReservationByManager(lno, bkno, id) {
	<sec:authorize access="hasAnyRole('ROLE_ANONYMOUS', 'ROLE_MEMBER')">
		if(confirm("대출예약 취소는 관리자만 가능합니다."))
			location.href="${pageContext.request.contextPath}/";
		else return;
	</sec:authorize>
	if (confirm("해당 "+ lno +"번 대출예약 신청 건을 취소하시겠습니까?")) {
		...
		var loans = {lno: lno, bkno: bkno, id: id};
 		loansService.cancelLoansReservationByManager(loans, function(result) {
			$("#LoadingLayerModal").modal('hide');
			alert(result);
			location.reload();
		})
